...
- **Session continuity** (`session_key`): a per-agent `AgentDef.session_key` opts an agent into reusing one Claude session across executions; without it every execution starts a fresh session (the historical behavior). This is what makes an intra-run loop-back keep the context it already built.
  - The provider holds `_session_ids: dict[tuple[session_key, cwd], session_id]` — keyed by the **workflow-authored key, not by agent name**, which is the entire point: several agents, or several executions of one agent, deliberately share a session. The **cwd is part of the key** because the CLI stores transcripts per working directory: two agents sharing a key under different directories cannot share a session, and keying on the label alone would make them overwrite each other's id so neither ever resumed. `_resolve_resume_session` prefers an id recorded this run over one restored from a checkpoint, then passes it as `ClaudeAgentOptions.resume`; `fork_session=False` is set **explicitly** (it matches the SDK default, but a fork would issue a new id and strand the map on a dead session).
  - Capture is deliberately narrow: only `AssistantMessage` / `ResultMessage` (`_SESSION_ID_MESSAGES`) update the map. Hook frames carry a `session_id` of their own — a `SessionStart` hook fires one before the first assistant turn — and recording it would shadow the real session with an id that has no transcript, permanently breaking continuity for the run. The capture sits *before* the interrupt and timeout checks, because an agent cut short mid-run is exactly the one whose accumulated context is worth resuming.
  - **The transcript guard is load-bearing, not defensive polish.** `--resume` against a transcript the CLI cannot find makes it abort with `ProcessError` *before running the agent* — so a naive passthrough would convert an ordinary first iteration, a pruned transcript (the CLI prunes on its own schedule), or a moved `working_dir` into a hard failure. `_session_transcript_exists` checks the **exact on-disk path the CLI uses**, `<CLAUDE_CONFIG_DIR or ~/.claude>/projects/<project_key_for_directory(cwd)>/<id>.jsonl`, rather than asking `get_session_info`, which answers a different question in both directions: it derives a *summary* from a bounded head+tail read and returns `None` whenever it cannot — a sidechain session, or a transcript whose first prompt overruns the head buffer with no title/last-prompt record in the tail — so a resumable session can look absent; and it falls back to scanning **sibling git worktrees** via a `git` subprocess (so it reports a session as present from a subdirectory or worktree the CLI would then refuse). `get_session_info` is still consulted as a fallback — it tolerates a hash mismatch for very long paths the exact check cannot model — but only when its recorded `cwd` `realpath`-matches ours. The whole lookup runs in `asyncio.to_thread` (the worktree fallback shells out with a 5s timeout, which would otherwise stall concurrent agents) and any exception degrades to "start fresh". Both symbols are imported in a **separate** `try` from the required SDK symbols, so an SDK below the lookup floor degrades just this guard instead of disabling the provider.
  - `checkpoint_resume` stays **`False`** even though a keyed agent's session *is* persisted and re-applied on resume. That flag is a blanket promise the startup banner reads out, and it would be false for every agent that has not opted in — which is all of them by default. `session_continuity=True` is the honest, granular claim; under-claiming the blanket one keeps the banner truthful for the majority who declare no key.
  - Checkpoint parity reuses the existing duck-typed hooks: `get_session_ids` / `set_resume_session_ids` match `copilot.py`, so `WorkflowEngine._write_checkpoint` and `cli/run.py` pick them up with **no CLI or checkpoint-schema change**. The `copilot_session_ids` field is an opaque flat `dict[str, str]` shared by every provider, and our key-space (author-chosen labels) genuinely collides with Copilot's (agent names) — `session_key: investigate` on an agent named `investigate` is the obvious thing to write. So our keys are namespaced *and* carry the cwd: `claude-agent-sdk:["<key>", "<cwd>"]` (`_SESSION_KEY_NAMESPACE` + `json.dumps([key, cwd])`). `set_resume_session_ids` skips entries that are not ours or fail to parse rather than raising. The engine's collection loop **merges every active provider's map** instead of stopping at the first one exposing `get_session_ids`, so a mixed-provider workflow persists all of them rather than keeping whichever provider's agent ran earliest. (`copilot` and `hermes` both key by bare agent name and can still shadow each other in a workflow mixing those two — pre-existing, and not something this change alters.)
  - `session_key` is rejected on `script` / `human_gate` / `workflow` / `wait` / `set` / `terminate` types (no provider session to continue); it is whitespace-stripped with `min_length=1`, and is **never Jinja2-rendered** — a `{{ ... }}` value is rejected by `validate_session_key_is_literal` rather than becoming one literal key shared by every iteration, which is the first thing a `for_each` author reaches for. It is gated on the `session_continuity` capability: declaring it against a provider that cannot honor it is a hard `conductor validate` error, alongside the existing `working_dir` / `skills` / `max_session_seconds` checks in `_check_agent_capabilities` (which already covers `for_each` inline agents too). The `sandbox:` block ships ungated, but that precedent does not transfer: `sandbox:` is provider-branded and unreachable unless you have configured `aca`, whereas `session_key` is a bare field sitting next to `model` and `retry` on every agent. Silently starting a fresh session is the same class of quietly-discarded operational intent the capability system exists to catch. Sharing a key across genuinely *concurrent* executions **is** rejected by `config/validator.py`, alongside the existing `concurrent_safe` checks that already walk the same groups: two parallel members with one key, or a keyed for-each agent with `max_concurrent > 1`. That is not a style nit — the second execution resumes a session the first still has open, orphaning it and leaving two `claude` processes appending to one transcript. It is provider-independent and statically decidable, so it is caught rather than documented.
  - User-facing documentation lives in `docs/workflow-syntax.md` (Session Continuity section); see also `examples/claude-agent-sdk-session-key.yaml`.
