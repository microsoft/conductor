...
- **plugins/**: Plugin resolution — the *plugin* as the unit of opt-in (issue #378)
  ...
  - `registry.py` - `resolve_plugins(entries, base_dir, home)` → `ResolvedPlugin` (skills, agents, mcp_servers, dropped, disabled). Reuses `skills/registry.py::is_path_entry` and `expand_skills_root` so plugins add no second opinion about what a path entry or a skill directory is
  - `errors.py` - `PluginError(ValueError)` base + `PluginNotFoundError` / `PluginManifestError`, mirroring `skills/errors.py`
