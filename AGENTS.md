...
- **skills/**: Skill registry and loader (opt-in, bundled skill content)
  ...
  - `loader.py` - Reads `SKILL.md` + `references/*.md` for providers that require eager preamble injection; wraps each skill in `<skill name="...">` tags inside a `<skills>` envelope. Bounded by `runtime.skill_injection`
