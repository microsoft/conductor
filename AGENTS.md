...
- **Skills** (issue #352): `supports_native_skills=True`. Skills are enabled through the SDK, not prompt injection, and three options move together in `execute`:
  - `plugins=[{"type": "local", "path": <plugin root>}]` + `skills=["<plugin>:<skill>"]`. The SDK has no skill-*directory* surface, so `_resolve_skill_plugins` maps each directory back to the plugin that owns it. A skill directory with no `.claude-plugin/plugin.json` above it raises `ProviderError` rather than being dropped — silently handing the agent less than the workflow declared is the failure mode worth being loud about.
  - `setting_sources=[]` **unconditionally**, for the same reason `strict_mcp_config=True` is unconditional a few lines away. Left unset, the CLI loads `~/.claude/skills`, every `.claude/skills` up the tree, `CLAUDE.md`, project `settings.json`, and hooks — none of it declared by the workflow, all of it varying by machine and launch directory. Conductor surfaces workspace instructions through its own opt-in `--workspace-instructions` instead. Note `skills=[]` and `skills=None` are **not** interchangeable upstream: `None` means "CLI defaults apply", so only the explicit empty list makes `skills: []` a real opt-out.
