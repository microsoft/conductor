...
- **providers/**: SDK provider abstraction
  ...
  - `_output_shape.py` - `normalize_agent_output(content, schema)` — the single entry point providers call before `validate_output` (issue #343). It raises `ValidationError` when the parsed response is not a JSON object (a bare `42`/`null`/array), which matters because `validate_output` would otherwise do `field_name not in content` on a scalar and raise an uncaught `TypeError`. It then applies `unwrap_scalar_wrappers`: fires only when the schema declares `string`/`number`/`boolean`, a `dict` arrived, and **exactly one** candidate under the field's own name or a generic `value`/`result` key has the expected type. Two matches count as ambiguous; any other key shape is ignored. Both are left untouched (same object identity) so the caller re-prompts rather than guessing — this is what stops `{"answer": {"error": "..."}}` being laundered into an answer. Every unwrap logs a warning naming the discarded keys. Kept out of `executor/output.py` on purpose — see the note there.
