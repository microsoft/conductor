...
- **Session continuity** (`session_key`): a per-agent `AgentDef.session_key` opts an agent into reusing one Claude session across executions; without it every execution starts a fresh session (the historical behavior). This is what makes an intra-run loop-back keep the context it already built, and it is also why `checkpoint_resume` is now `True`.
  - The provider holds `_session_ids: dict[session_key, session_id]` — keyed by the **workflow-authored key, not by agent name**, which is the entire point: several agents, or several executions of one agent, deliberately share a session. `_resolve_resume_session` prefers an id recorded this run over one restored from a checkpoint, then passes it as `ClaudeAgentOptions.resume`.
  - Capture is opportunistic — any SDK message carrying a `session_id` updates the map, not just `ResultMessage`. An agent that dies mid-run is precisely the one whose accumulated context is worth resuming, and `fork_session` defaults to `False` so a resumed session keeps its id and the map stays stable.
  - **The existence check is load-bearing, not defensive polish.** `--resume` against a transcript the CLI cannot find makes it abort with `ProcessError` *before running the agent* — so a naive passthrough would convert an ordinary first iteration, a pruned transcript (the CLI prunes on its own schedule), or a moved `working_dir` into a hard failure. `get_session_info(sid, directory=cwd)` returns `None` rather than raising and is cwd-scoped, so one cheap stat covers all three cases; a miss logs a warning and starts fresh. `get_session_info` is exported at the pinned floor (`>=0.2.82`), and the `None` guard keeps an SDK that lacks it from crashing.
  - Checkpoint parity reuses the existing duck-typed hooks: `get_session_ids` / `set_resume_session_ids` match `copilot.py`, so `WorkflowEngine._write_checkpoint` and `cli/run.py` pick them up with **no engine, CLI, or checkpoint-schema change**. The `copilot_session_ids` field is an opaque `dict[str, str]`; nothing downstream interprets the keys, so a key-space of `session_key` values round-trips fine. Note that engine loop `break`s at the first provider exposing `get_session_ids`, so a workflow mixing providers persists only one map — pre-existing, and a foreign key-space simply misses and starts fresh.
  - `session_key` is rejected on `script` / `human_gate` / `workflow` / `wait` / `set` / `terminate` types (no provider session to continue), and is `min_length=1`. Like `SandboxConfig`, it validates structurally under any provider but is consumed only by this one — deliberately no capability flag or `validator.py` gating, matching the `sandbox:` precedent. Sharing a key across genuinely concurrent executions (parallel members, for-each with `max_concurrent > 1`) interleaves turns in one session and is documented as an authoring error rather than blocked.
  - See `examples/claude-agent-sdk-session-key.yaml` and `docs/providers/experimental.md` (Session Continuity section).
