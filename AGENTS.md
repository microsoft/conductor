...
- **providers/**: SDK provider abstraction
  ...
  - `_output_shape.py` - `unwrap_scalar_wrappers(content, schema)` — conservative, provider-side normalization of wrapper-shaped scalar output (issue #343). Fires only when the schema declares `string`/`number`/`boolean`, a `dict` arrived, and one unambiguous candidate inside it already has the expected type: a key matching the field name, then a `value`/`result` key, then a sole key. Anything else is returned untouched (same object identity) so the caller re-prompts rather than guessing. Every unwrap logs a warning. Kept out of `executor/output.py` on purpose — see the note there.
