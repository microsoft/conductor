...
server-scoped permission rule each (`mcp__<server>__*`); server names that
cannot form an unambiguous rule, and plugin subagents that would have no
dispatch tool, are refused at `conductor validate` and at run time. An
