...
- **Automatic client-side context compaction for pydantic-ai providers** (`claude` and `openai`). Conductor automatically condenses the conversation history when it crosses a calculated trigger threshold. This threshold is computed using an additive reserve formula, keeping headroom for the output limit and a tool-output-derived buffer. Additionally, this update unifies the default `max_tokens` to 16384 for both providers and removes the old 64k fallback. The `openai` provider now always sends the `max_tokens` parameter to the API. For Claude thinking agents, `low` or `medium` effort levels without an explicit `max_tokens` limit now send 16384 tokens instead of the former 8192 or 12288 tokens. To condense the history, the compaction process runs a three-stage strategy: clearing old tool results first, summarizing older messages with a nested model call, and sliding the window as a deterministic fallback. Users can still override the resolved context window size using the `CONDUCTOR_COMPACTION_CONTEXT_WINDOW` environment variable.
