...
The `claude-agent-sdk` provider bridges MCP servers into the CLI, but not per-agent tool allowlists. Which *built-in* Claude Code tools an agent gets is chosen by `runtime.provider.native_tools`, not by the agent's `tools:` list. Concretely:

- Omitting `tools:` entirely (with no workflow-level `tools:`) — governed by `native_tools`:
  - `none` (the default, including for the bare `provider: claude-agent-sdk` shorthand) — **no built-in tools**: no filesystem, shell, web or editing. Unapproved tool calls are denied rather than prompted for (`dontAsk`). The `Skill` loader is granted back when the agent has skills enabled.
  - `claude_code` — the full `claude_code` preset (filesystem read/write, shell, web, editing) with permissions approved automatically, matching the bare `claude` CLI. `working_dir` scopes where paths resolve; it is **not a sandbox**. The run prints a one-time warning saying so.
- `runtime.mcp_servers` — **supported** under either value. Only declared servers attach: Conductor sets `strict_mcp_config`, so ambient Claude Code MCP settings are ignored. Under `none`, each declared server is pre-approved by a single server-scoped rule (`mcp__<server>__*`) and nothing broader, so an MCP-only agent works with no built-in tools. Server names must then use only letters, digits, `-` and single `_` characters (not leading or trailing); anything else is refused rather than rewritten. A `.` is refused too, because the CLI rewrites it to `_` when building tool names (`fs.tools-2` exposes `mcp__fs_tools-2__…`), so a dotted rule would never match. An MCP tool that requires interactive user approval may be denied under `none`. A narrowing per-server `tools:` filter is refused, since the SDK has no equivalent field.
- Per-agent `tools: []` — disables the built-in tools for that agent under **either** `native_tools` value, except the `Skill` loader when the agent has skills enabled (an empty tool set would otherwise leave a declared skill unreachable). Declared MCP servers would still attach, so this combination is refused at `conductor validate` and again at run time (before any MCP config is written) whenever the agent would get servers from `runtime.mcp_servers` or its plugins — omit `tools:` instead for an MCP-only agent.
...
- Workflow-level `tools:` combined with an agent that omits `tools:` — **rejected at `conductor validate`**. The agent would otherwise inherit that non-empty list at runtime and hit the same refusal with a confusing message. Remove the workflow-level `tools:` (so omitting `tools:` falls back to `native_tools`) or set the agent's `tools: []`.
- Plugins that ship **subagents** — need a dispatch tool, which only `native_tools: claude_code` provides. Under `none` (or with `tools: []`) they are refused at validate and at run time rather than registered unreachable. Either set `native_tools: claude_code`; or disable the plugin's subagents with `agents: false` — together with `skills: false` if that plugin's skills are enabled, because reaching its skills registers the plugin root, which exposes its subagents again (so `agents: false` alone is refused there); or run the agent on `copilot`.
