...
/**
 * Graph node for `type: set` workflow steps.
 *
 * Mirrors `ScriptNode` visually but swaps the Terminal icon for `Variable`
 * (since `set` steps bind named values) and surfaces set-specific stats
...
export const SetNode = memo(function SetNode({ data, selected }: NodeProps) {
  ...
  // Build stats line: prefer key count for multi-binding, fall back to
  // a short value preview for single-binding scalars.
  const statsLine = (() => {
    ...
    if (status === 'completed') {
      ...
      if (outputKeys && outputKeys.length > 0) {
        parts.push(`${outputKeys.length} key${outputKeys.length === 1 ? '' : 's'}`);
      } else if (valueRepr) {
        const preview = valueRepr.length > 24 ? valueRepr.slice(0, 21) + '…' : valueRepr;
        parts.push(preview);
      }
    ...
    <>
      ...
      >
        ...
        >
          ...
          >
            <Variable className="w-3.5 h-3.5" style={{ color: borderColor }} />
          </div>
